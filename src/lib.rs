...
//!   `CordBuffer`. Building with `default-features = false` gives a crate using
//!   `no_std` and `alloc` (with any global allocator); everything else —
//!   including `core::fmt::Write` for [`Cord`] — stays available. `no_std`
//!   targets need 32-bit and pointer-width atomics (`target_has_atomic = "32"`
//!   and `"ptr"`).
