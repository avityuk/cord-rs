...
pub(crate) unsafe fn cord_collect_reps(rep: *mut CordRep) -> Vec<*mut CordRep> {
    cord_collect_reps_if(|_| true, rep)
}
...
pub(crate) unsafe fn make_hex_flat(i: usize) -> *mut CordRep {
    make_flat(format!("0x{i:04x}").as_bytes())
}
...
pub(crate) unsafe fn make_tree(size: usize, append: bool) -> *mut CordRepBtree {
...
pub(crate) unsafe fn create_tree(reps: &[*mut CordRep]) -> *mut CordRepBtree {
    cord_rep_btree_from_flats(reps)
}

/// `CreateTree(data, chunk_size)`: a tree of flats of `chunk_size` bytes.
pub(crate) unsafe fn create_tree_from_string(data: &[u8], chunk_size: usize) -> *mut CordRepBtree {
    create_tree(&create_flats_from_string(data, chunk_size))
}

/// `CreateTreeReverse(data, chunk_size)`: like [`create_tree_from_string`]
/// but built by prepending the flats in reverse order.
pub(crate) unsafe fn create_tree_reverse(data: &[u8], chunk_size: usize) -> *mut CordRepBtree {
